...
public class JdbcProductStore implements Store {
  ...
  public Single<JsonObject> createFormRequest(JsonObject item) {
    ...
    // JsonArray userApproval = new JsonArray((String) item.getValue("user_approval"));\

    // JsonArray userApproval = (JsonArray) item.getValue("user_approval");
    
    // System.out.println(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>"+userApproval.size());
  ...
  //       return conn.rxQueryStreamWithParams(LOGIN, params)
  //         .flatMapCompletable(up -> {
  //           System.out.println(">>>>>>>>>>>..."+up.columns());
  //           if (up.columns() == null ) {
  //             return Completable.error(new NoSuchElementException("Username doesn't exist'"));
  //           }
  //           return Completable.complete();
  //         })
  //         .doAfterTerminate(conn::close);
